...
`prisma/schema.prisma` cobre identidade (Better Auth), tenancy (`Conta`/`Membership`/`Propriedade`) e os cadastros já migrados (`Cultura`, `Safra`, `Socio`, `Supplier`/`CompraFornecedor`, `QuadroSafra`, `ContratoBancario`/`Parcela`, `Aquisicao`, `ContratoArrendamento`, `ContratoComercial`, `EmpresaPJ`, `BalancoPatrimonial`, `Cotacao`). Leitura via Server Components (`src/server/*` chamado direto de `page.tsx`), escrita via server actions (`'use server'` em `src/server/*`, chamadas pelos Drawers). Nunca importar `@prisma/client` direto em client component.
