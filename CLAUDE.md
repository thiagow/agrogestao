...
- **`vitest`** para os testes (`npm test`, config em `vitest.config.mts`) — cobre **só as funções puras do domínio financeiro** (`src/lib/*.test.ts`: `amortizacao`, `taxa-efetiva`). Nada de banco, rede ou React nos testes: são os cálculos que decidem projeção de dívida, e é neles que erro silencioso custa caro. Verificação que exige I/O real fica em `scripts/verificar-juros-indexados.ts` (`npx tsx`), que é conferência manual, não gate.
