...
6. **Tabela oficial de códigos IRPF em Bens e Direitos.** `BemDireito.codigoTipo` (05/08/2026, ver `CLAUDE.md`) é texto livre em vez de um select dependente do `grupoIrpf` porque não temos a tabela completa de códigos por grupo da Declaração de Bens e Direitos da Receita Federal confirmada — só o exemplo do print original do AgroFlow, "18 — Imóvel Rural" (Grupo "Bens Imóveis"). Quando o usuário tiver a tabela oficial completa (ou os códigos exatos usados pelo AgroFlow para os demais grupos — Bens Móveis, Participações Societárias, Aplicações e Investimentos, Depósitos à Vista e Poupança, Créditos e Outros Direitos, Criptoativos, Outros), trocar `codigoTipo` de `Input` texto livre (`BemDireitoDrawer.tsx`) para um `Select` dependente do grupo escolhido — mantendo o dado já salvo como texto compatível (é string em ambos os casos, não precisa migration).

Cada um desses seis itens deveria começar por uma rodada de perguntas ao usuário antes de qualquer código — são decisões de produto (cobrar ou não, qual storage, qual conteúdo no relatório, qual tabela de códigos), não apenas técnicas.
